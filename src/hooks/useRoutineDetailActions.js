...
export function useRoutineDetailActions({
  ...
  const focusExercise = (index, delay = 50) => {
    ...
    if (exercise) {
      setTimeout(() => {
        ...
        exerciseRefs.current[exercise.id]?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
      }, delay);
    }
  };
  ...
  const focusExerciseGroupById = (id, delay = 0) => {
    setTimeout(() => {
      ...
      exerciseGroupRefs.current[id]?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
    }, delay);
  };
