...
import type { Id, SessionExerciseGroup, Session } from "../types/workout";

export interface UseRoutineDetailActionsProps {
  sessionRefs: React.MutableRefObject<Record<Id, HTMLElement | null>>;
  exerciseRefs: React.MutableRefObject<Record<Id, HTMLElement | null>>;
  exerciseGroupRefs: React.MutableRefObject<Record<Id, HTMLElement | null>>;
  exerciseGroupRowRefs: React.MutableRefObject<Record<Id, HTMLElement | null>>;
  settingControlRefs: React.MutableRefObject<Array<HTMLElement | null>>;
  addSessionBtnRef: React.RefObject<HTMLButtonElement | null>;
  addExerciseBtnRef: React.RefObject<HTMLButtonElement | null>;
  addGroupBtnRef: React.RefObject<HTMLButtonElement | null>;
  pendingFocusIndexRef: React.MutableRefObject<number | null>;
  pendingGroupFocusIdRef: React.MutableRefObject<Id | null>;
}
...
export function useRoutineDetailActions({
  ...
  // --- Local States ---
  const [selectedRoutineId, setSelectedRoutineId] = useState<Id | null>(null);
  const [selectedSessionId, setSelectedSessionId] = useState<Id | null>(null);
  const [selectedExerciseId, setSelectedExerciseId] = useState<Id | null>(null);
  const [selectedExerciseGroupId, setSelectedExerciseGroupId] = useState<Id | null>(null);
  const [settingsMode, setSettingsMode] = useState<"exercise" | "group">("exercise");
  ...
  const [focusedRoutinePanel, setFocusedRoutinePanel] = useState<string | null>(null);
  ...
  const [editingSessionId, setEditingSessionId] = useState<Id | null>(null);
  const [editingSessionNameVal, setEditingSessionNameVal] = useState("");
  const [pendingNewSessionId, setPendingNewSessionId] = useState<Id | null>(null);
  const [pendingNewSessionReturnId, setPendingNewSessionReturnId] = useState<Id | null>(null);
  ...
  // --- Action Handlers ---
  const handleSelectRoutine = (id: Id | null) => {
  ...
  const handleSelectSession = (id: Id | null) => {
  ...
  const handleDuplicateRoutine = (routineId: Id) => {
  ...
  const finishSessionEdit = (session: Session) => {
    if (isReadOnly) return;
  ...
  const cancelSessionEdit = (session: Session) => {
  ...
  const handleAddExerciseToSession = (dictExercise: any) => {
  ...
  const handleDeleteExercise = (id: Id) => {
    if (isReadOnly) return;
  ...
  const handleUpdateTarget = (id: Id, field: string, value: any) => {
    if (isReadOnly) return;
    updateSessionExercise(id, { [field]: value });
  };

  const handleUpdateExerciseGroup = (id: Id, updates: Partial<SessionExerciseGroup>) => {
    if (isReadOnly) return;
  ...
  const handleSelectExercise = (id: Id | null) => {
    setSelectedExerciseId(id);
    if (id) setSettingsMode("exercise");
    if (id) setSelectedExerciseGroupId(null);
  };

  const handleSelectExerciseGroup = (groupOrId: SessionExerciseGroup | Id | null) => {
  ...
  const handleFocusExerciseGroupRow = (group: SessionExerciseGroup | null) => {
    setFocusedRoutinePanel("groups");
    if (!group?.id) return;
  ...
  const getFirstExerciseInGroup = (group: SessionExerciseGroup) => {
    return getLinksCoveredByGroup(activeSessionExercises, group)[0] || null;
  };

  const focusGroupSettings = (group: SessionExerciseGroup) => {
  ...
  const focusExerciseSettingsFromGroup = (group: SessionExerciseGroup) => {
    const firstExercise = getFirstExerciseInGroup(group);
    if (!selectedExerciseId && firstExercise) {
      setSelectedExerciseId(firstExercise.id);
    }
  ...
  const handleAddGroupButtonKeyDown = (event: React.KeyboardEvent<HTMLElement>) => {
  ...
  const handleAddExerciseGroup = ({ name, size }: { name: string; size: number }) => {
