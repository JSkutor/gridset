...
import { MAX_GROUPS_PER_SESSION, MIN_GROUP_SIZE, GROUP_COLOR_PALETTE } from '../../utils/sessionExerciseGroups.js';
...
describe('routineSlice: Routine CRUD', () => {
  ...
  test('updateRoutine changes routine name', () => {
    ...
    const updated = useWorkoutStore.getState().routines.find(rt => rt.id === r.id);
  ...
  test('deleteRoutine removes routine, its sessions, exercises, and groups', () => {
    ...
    assert.equal(useWorkoutStore.getState().routines.some(rt => rt.id === r.id), false);
    assert.equal(useWorkoutStore.getState().sessions.some(sn => sn.routine_id === r.id), false);
    assert.equal(useWorkoutStore.getState().sessionExercises.some(se => se.session_id === s.id), false);
    assert.equal(useWorkoutStore.getState().sessionExerciseGroups.some(g => g.session_id === s.id), false);
  });
  ...
  test('duplicateRoutine deep copies sessions, exercises, and groups with new IDs', () => {
    ...
    const link1 = useWorkoutStore.getState().addSessionExercise(s.id, bench.id, 1, 4, '8');
    const link2 = useWorkoutStore.getState().addSessionExercise(s.id, squat.id, 2, 3, '10');
    useWorkoutStore.getState().updateSessionExercise(link1.id, { rest_between_sets: 60, rest_after_exercise: 180 });
    const group = useWorkoutStore.getState().addSessionExerciseGroup(s.id, '슈퍼세트', 2);
    ...
    const copiedSessions = useWorkoutStore.getState().sessions.filter(sn => sn.routine_id === copy.id);
    assert.equal(copiedSessions.length, 1);
    assert.notEqual(copiedSessions[0].id, s.id);
    assert.equal(copiedSessions[0].name, 'Day A');

    const copiedLinks = useWorkoutStore.getState().sessionExercises.filter(se => se.session_id === copiedSessions[0].id);
    assert.equal(copiedLinks.length, 2);

    const copiedBenchLink = copiedLinks.find(se => se.exercise_id === bench.id);
    ...
    const copiedGroups = useWorkoutStore.getState().sessionExerciseGroups.filter(g => g.session_id === copiedSessions[0].id);
  ...
  test('duplicateRoutine returns null for non-existent source routine', () => {
    const result = useWorkoutStore.getState().duplicateRoutine('non-existent-id');
...
describe('routineSlice: Session CRUD', () => {
  ...
  test('updateSession changes session name', () => {
    ...
    const updated = useWorkoutStore.getState().sessions.find(sn => sn.id === s.id);
  ...
  test('deleteSession cascades links, groups, and compacts remaining session orders', () => {
    ...
    const remainingSessions = useWorkoutStore.getState().sessions
      .filter(sn => sn.routine_id === r.id)
      .sort((a, b) => a.session_order - b.session_order);

    // s2(두번째)가 지워졌으므로 s1(1), s3(2) → s3의 order가 2로 컴팩트
    assert.deepEqual(
      remainingSessions.map(sn => ({ id: sn.id, session_order: sn.session_order })),
      [
    ...
    assert.equal(useWorkoutStore.getState().sessionExercises.some(se => se.session_id === s2.id), false);
  });
  ...
  test('createTemporarySession returns existing temporary session if already present', () => {
    ...
    const ts1 = useWorkoutStore.getState().createTemporarySession(r.id, '첫 임시');
    const ts2 = useWorkoutStore.getState().createTemporarySession(r.id, '두번째 임시');
  ...
  test('reorderSessions updates session_order based on provided ID array', () => {
    ...
    const sessions = useWorkoutStore.getState().sessions
      .filter(sn => sn.routine_id === r.id)
      .sort((a, b) => a.session_order - b.session_order);
    assert.deepEqual(sessions.map(sn => sn.id), [s3.id, s1.id, s2.id]);
    assert.deepEqual(sessions.map(sn => sn.session_order), [1, 2, 3]);
  });
});
...
describe('routineSlice: Session Exercise CRUD', () => {
  test('addSessionExercise creates an exercise link with default rest times', () => {
    ...
    const bench = exerciseByLabel('벤치프레스');
    const link = useWorkoutStore.getState().addSessionExercise(s.id, bench.id, 1, 4, '8');
  ...
  test('deleteSessionExercise compacts remaining exercise orders inside the session', () => {
    const r = useWorkoutStore.getState().addRoutine('R');
    const s = useWorkoutStore.getState().addSession(r.id, 'S');
    const [bench, squat, deadlift] = ['벤치프레스', '스쿼트', '데드리프트'].map(exerciseByLabel);
    const first = useWorkoutStore.getState().addSessionExercise(s.id, bench.id, 1, 3, '10');
    const second = useWorkoutStore.getState().addSessionExercise(s.id, squat.id, 2, 3, '10');
    const third = useWorkoutStore.getState().addSessionExercise(s.id, deadlift.id, 3, 3, '10');
  ...
  test('updateSessionExercise modifies individual exercise link properties', () => {
    ...
    const s = useWorkoutStore.getState().addSession(r.id, 'S');
    const bench = exerciseByLabel('벤치프레스');
    const link = useWorkoutStore.getState().addSessionExercise(s.id, bench.id, 1, 3, '10');
    ...
    const updated = useWorkoutStore.getState().sessionExercises.find(se => se.id === link.id);
  ...
  test('reorderSessionExercises updates orders and re-applies group target sets', () => {
    ...
    const s = useWorkoutStore.getState().addSession(r.id, 'S');
    const [bench, squat, deadlift] = ['벤치프레스', '스쿼트', '데드리프트'].map(exerciseByLabel);
    const first = useWorkoutStore.getState().addSessionExercise(s.id, bench.id, 1, 4, '8');
    const second = useWorkoutStore.getState().addSessionExercise(s.id, squat.id, 2, 3, '10');
    const third = useWorkoutStore.getState().addSessionExercise(s.id, deadlift.id, 3, 2, '5');

    // Create a group covering positions 2-3 (squat, deadlift)
    useWorkoutStore.getState().addSessionExerciseGroup(s.id, '그룹', 2);

    // Reorder: move bench to position 3
    useWorkoutStore.getState().reorderSessionExercises(s.id, [second.id, third.id, first.id]);

    const exercises = useWorkoutStore.getState().sessionExercises
      .filter(se => se.session_id === s.id)
      .sort((a, b) => a.order - b.order);
...
describe('routineSlice: Session Exercise Groups', () => {
  test('addSessionExerciseGroup creates a group that unifies target_sets and rest times', () => {
    ...
    const group = useWorkoutStore.getState().addSessionExerciseGroup(s.id, '슈퍼세트 A', 2);
    ...
    const groupedExercises = useWorkoutStore.getState().sessionExercises
      .filter(se => [first.id, second.id].includes(se.id));
    assert.deepEqual(groupedExercises.map(se => se.target_sets), [4, 4]);
    assert.deepEqual(groupedExercises.map(se => se.rest_between_sets), [60, 60]);
    assert.deepEqual(groupedExercises.map(se => se.rest_after_exercise), [150, 150]);
  });
  ...
  test('addSessionExerciseGroup returns null when MAX_GROUPS_PER_SESSION is exceeded', () => {
    ...
    exercises.forEach((ex, i) => {
      useWorkoutStore.getState().addSessionExercise(s.id, ex.id, i + 1, 3, '10');
    });

    const groups = Array.from({ length: MAX_GROUPS_PER_SESSION }, (_, i) =>
      useWorkoutStore.getState().addSessionExerciseGroup(s.id, `그룹 ${i + 1}`, 2),
    );
    assert.equal(groups.every(Boolean), true);
    assert.equal(groups.length, MAX_GROUPS_PER_SESSION);

    const overflow = useWorkoutStore.getState().addSessionExerciseGroup(s.id, '그룹 5', 2);
    assert.equal(overflow, null);
  });
  ...
  test('addSessionExerciseGroup returns null when not enough exercises', () => {
    ...
    const group = useWorkoutStore.getState().addSessionExerciseGroup(s.id, '그룹', 2);
  ...
  test('addSessionExerciseGroup assigns colors from palette sequentially', () => {
    ...
    assert.deepEqual(groups.map(g => g.color), GROUP_COLOR_PALETTE);
    assert.deepEqual(groups.map(g => g.start_order), [1, 3, 5, 7]);
  });
  ...
  test('addSessionExerciseGroup places groups at first non-overlapping position', () => {
    ...
    const g1 = useWorkoutStore.getState().addSessionExerciseGroup(s.id, 'G1', 2);
    assert.equal(g1.start_order, 1);

    const g2 = useWorkoutStore.getState().addSessionExerciseGroup(s.id, 'G2', 2);
  ...
  test('updateSessionExerciseGroup moves group position avoiding overlaps', () => {
    ...
    const g1 = useWorkoutStore.getState().addSessionExerciseGroup(s.id, '첫 그룹', 2);
    const g2 = useWorkoutStore.getState().addSessionExerciseGroup(s.id, '둘째 그룹', 2);

    // Move g1 to position 2 — it should jump to position 5 (after g2)
    const moved = useWorkoutStore.getState().updateSessionExerciseGroup(g1.id, { start_order: 2 });
  ...
  test('updateSessionExerciseGroup propagates target_sets/rest to all grouped exercises', () => {
    ...
    const linkInGroup = useWorkoutStore.getState().sessionExercises
      .filter(se => se.session_id === s.id)
      .sort((a, b) => a.order - b.order)[0];
    useWorkoutStore.getState().updateSessionExercise(linkInGroup.id, { target_sets: 6, rest_between_sets: 75 });

    const groupedLinks = useWorkoutStore.getState().sessionExercises
      .filter(se => se.session_id === s.id)
      .sort((a, b) => a.order - b.order);
  ...
  test('updateSessionExerciseGroup returns null for non-existent group', () => {
    const result = useWorkoutStore.getState().updateSessionExerciseGroup('non-existent', { name: 'Nope' });
    assert.equal(result, null);
  });
  ...
  test('deleteSessionExerciseGroup removes the group', () => {
    ...
    assert.equal(useWorkoutStore.getState().sessionExerciseGroups.some(g => g.id === group.id), false);
  });
  ...
  test('deleteSessionExercise removes undersized groups', () => {
    ...
    const s = useWorkoutStore.getState().addSession(r.id, 'S');
    const [bench, squat] = ['벤치프레스', '스쿼트'].map(exerciseByLabel);
    const first = useWorkoutStore.getState().addSessionExercise(s.id, bench.id, 1, 3, '8');
    useWorkoutStore.getState().addSessionExercise(s.id, squat.id, 2, 3, '10');
    const group = useWorkoutStore.getState().addSessionExerciseGroup(s.id, '슈퍼세트', 2);
  ...
  test('exercises still exist after undersized group removal', () => {
    ...
    assert.equal(useWorkoutStore.getState().sessionExercises.some(se => se.id === second.id), true);
    // And its order should be compacted to 1
    const remaining = useWorkoutStore.getState().sessionExercises.find(se => se.id === second.id);
