...
export default function SetRecordTable({ records, exercise, compact = false }: { records: SetRecord[]; exercise?: Partial<Exercise>; compact?: boolean }) {
  const hasMemo = true;
        ...
        <tbody>
          {records.map((record) => (
            <tr key={record.id}>
              ...
              <td className="cell-value">{String(formatSetCellValue(record.weight) ?? '')}</td>
              <td className="cell-value">{String(formatSetCellValue(record.record) ?? '')}</td>
              {hasMemo && (
