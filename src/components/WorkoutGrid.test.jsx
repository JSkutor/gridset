...
function setupWorkoutGrid() {
  ...
  render(
    React.createElement(WorkoutGrid, {
      session,
...
describe("WorkoutGrid workout start timing", () => {
  test("starts the workout from the first table input and saves reps-only rows", () => {
    ...
    const savedRecord = useWorkoutStore
      ...
      .setRecords.find(
        (record) =>
          record.workout_log_id === savedLog.id &&
          record.exercise_id === bench.id,
      );
...
describe("WorkoutGrid bodyweight equipment", () => {
  test("disables kg input for 맨몸 exercises and Tab skips to reps", async () => {
    ...
    // ArrowLeft from reps moves focus back to weight (a read-only cell)
    await user.keyboard("{ArrowLeft}");
    expect(document.activeElement).toBe(weightInput);

    // weight input is readOnly — onKeyDown prevents non-navigation key input
    // Note: jsdom may still set .value despite readOnly; the guard in
    // onKeyDown calls preventDefault + resets value to block the change.
    await user.keyboard("12");
    // The onKeyDown guard resets value and prevents action
    // (React re-render will restore controlled value)
...
describe("WorkoutGrid memo entry", () => {
  test("saves memo typed in the set memo textarea with the focused set record", async () => {
    ...
    const savedRecord = useWorkoutStore
      .getState()
      .setRecords.find(
        (record) => record.exercise_id === bench.id && record.set_number === 1,
      );
  ...
  test("keeps memos attached to the set that was focused before opening the memo textarea", async () => {
    ...
    await user.type(
      screen.getByPlaceholderText("이 세트에 대한 메모..."),
      "첫 세트 메모",
    );
    ...
    const savedRecords = useWorkoutStore
      .getState()
      .setRecords.filter((record) => record.exercise_id === bench.id)
      .sort((a, b) => a.set_number - b.set_number);

    expect(savedRecords.map((record) => record.memo)).toEqual([
      "첫 세트 메모",
      "두 번째 메모",
    ]);
  });
