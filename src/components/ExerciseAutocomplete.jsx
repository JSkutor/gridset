...
import { matchHangul, extractChosung, disassembleHangul, isChosungOnly } from '../utils/hangul';
...
export default function ExerciseAutocomplete({ onSelect, placeholder = '운동 검색 (예: 풀업, 벤치, ㅍㅇ, OHP)', autoFocus = false, onCancel }) {
  ...
  useEffect(() => {
    ...
    // 2. 정확도(relevance) 점수 계산 및 정렬
    const sorted = matched.map(exercise => {
      ...
      // B. 자소 분리 기준 매칭 (한글 입력 중간 상태 및 일반 문자열 매칭)
      const nameDis = disassembleHangul(name);
      if (nameDis.startsWith(qDisassembled)) {
        score += 1000; // 입력한 글자로 시작함
      } else if (nameDis.includes(qDisassembled)) {
        score += 500;  // 입력한 글자를 포함함
      }

      // C. 초성 검색인 경우 (자음만 입력한 경우)
      if (qChosung) {
        const nameCho = extractChosung(name);
        if (nameCho.startsWith(qChosung)) {
          score += 800;  // 초성으로 시작함
        } else if (nameCho.includes(qChosung)) {
          score += 400;  // 초성 포함함
        }
      }
      ...
      if (exercise.synonyms && exercise.synonyms.length > 0) {
        for (const syn of exercise.synonyms) {
          ...
          const synDis = disassembleHangul(synLower);
          
          if (synLower === q) {
            score += 300;
            break;
          } else if (synDis.startsWith(qDisassembled)) {
            score += 150;
            break;
          } else if (synDis.includes(qDisassembled)) {
            score += 50;
            break;
          }
        }
      ...
      // 2순위: 글자 수가 짧은 순 (검색어와 글자 수가 근접할수록 정확함)
