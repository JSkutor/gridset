...
- **Data Persistence Migration**: Schema migration logic is isolated in `src/store/workoutPersistenceMigration.ts`.
...
- **데이터 영속성 마이그레이션**: 스키마 마이그레이션 로직은 `src/store/workoutPersistenceMigration.ts`에 분리되어 있다.
