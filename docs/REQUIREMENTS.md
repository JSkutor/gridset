...
### 7.4 Data Export
- Signed-in and guest users can export workout data from the account menu (`AccountMenu`).
- Export produces a ZIP archive of CSV files (routines, sessions, session exercises, groups, exercises, workout logs, set records) via `src/utils/exportData.ts` (JSZip).
- Export is optional at runtime but JSZip is currently bundled in the main chunk because `AccountMenu` statically imports the utility.
...
- **Utility Logic Separation**: Pure utility logic (calculations, formats) lives in `src/utils/` (mostly TypeScript).
- **State Store**: Zustand + `persist` in `src/store/useWorkoutStore.ts`, four slices in `src/store/slices/*.ts`, shared types in `src/store/types.ts`.
- **Data Persistence Migration**: `src/store/workoutPersistenceMigration.ts` (persist version **1**, pass-through only; storage key `gridset-workout-v1`).
- **Repository Pattern**: Supabase read/write in `src/api/supabaseWorkoutRepository.ts`.
- **Exercise Catalog**: `src/data/exerciseDictionary.ts` (875 entries, offline). `scratch/extracted_exercises.json` is a generation source, not loaded at runtime.
- **Custom Hooks** (representative):
  - `useGlobalShortcuts.ts`, `useTabNavigation.ts`, `useGridNavigation.ts` — keyboard UX.
  - `useWorkoutSessionRotation.ts`, `useAuthSessionBridge.ts`, `useWorkoutDraft.ts`.
  - `useRoutineDetailActions.js`, `useRoutineKeyboardNavigation.js` — still JavaScript (TypeScript migration pending).
- **UI Layer**: React components remain mostly `.jsx` under `src/components/`; `App.jsx` statically imports all three main tabs (no route-based code splitting).
- **Sync Failure UI**: `src/components/SyncStatusBanner.jsx`.
- **Demo Data**: `src/data/dummyGenerator.ts`.
- **TypeScript adoption**: `allowJs: true`, `checkJs: false`, `strict: true` in `tsconfig.json`. Production store/api/utils/data paths are TS; components and some routine hooks are not.
...
- **Performance / bundle size**:
  - Lazy-load the exercise dictionary (fetch JSON or split chunk) instead of inlining ~875 entries in the main bundle.
  - `React.lazy` (or equivalent) for Log and Routine tab trees when Set-only usage is common.
  - Dynamic `import()` for JSZip in export flow.
  - Vite manual chunks or Rolldown code splitting once tab boundaries are stable.

## 13. Performance And Initial Load

Current production build characteristics (Vite 8, no custom `build.rollupOptions`):

| Asset | Approx. size |
| :---- | :------------- |
| Main JS chunk | ~1.1 MB minified (~290 KB gzip) |
| CSS | ~80 KB |

**Why first load feels heavy**

1. **Single JS entry** — `main.jsx` → `App.jsx` pulls Routine, Set, and Log UIs, completion modal, auth, and store in one graph. There is no lazy route or tab split.
2. **Exercise dictionary in the main chunk** — `EXERCISE_DICTIONARY` is a large const array in `exerciseDictionary.ts`. It is required by `exerciseSlice`, `exerciseSearch`, `workoutPersistenceMigration`, `dummyGenerator` / `EXERCISE_CATALOG`, and `supabaseWorkoutRepository`, so it cannot be tree-shaken away.
3. **Guest bootstrap** — First visit hydrates Zustand from `localStorage` and may run migrations; new guests also seed demo routines/logs (CPU + storage I/O after JS parse).
4. **Always-on dependencies** — `@supabase/supabase-js`, `framer-motion` (routine lists), `jszip` (export), `lucide-react`, React 19.

**Deployment**

- Host as a static SPA (`dist/`). Recommended platform: **Vercel** (native Vite workflow, env vars for `VITE_*`, preview deployments). Netlify is equivalent. GitHub Pages works but needs `base` path and Actions setup and is less convenient for Supabase env per environment.
- See [README.md](../README.md) deployment section for Vercel-oriented steps.

**Acceptance for 1.0 web release**

- Heavy first load is a known tradeoff for offline search over 875 exercises and a single-bundle deploy; it is documented, not a release blocker, unless product targets very slow networks without a follow-up optimization milestone.
...
### 7.4 데이터보내기
- 계정 메뉴에서 루틴·세션·운동·로그·세트 기록을 ZIP(CSV 묶음)으로 다운로드한다 (`exportData.ts`).
...
- **게스트에서 회원으로 전환**: 로그인 시 서버에 기록이 있으면 서버 데이터가 우선한다. 서버가 비어 있고 게스트가 데모를 정리한 뒤(`hasClearedDemoData`) 만든 로컬 데이터만 업로드될 수 있다. 자세한 규칙은 [SYNC_POLICY.md](SYNC_POLICY.md).
- **원격 동기화 실패 UX**:
...
## 11. 현재 아키텍처 노트
- **유틸리티**: `src/utils/` (대부분 TypeScript).
- **스토어**: `src/store/useWorkoutStore.ts`, 슬라이스 `src/store/slices/*.ts`, persist 버전 1 (`gridset-workout-v1`).
- **API**: `src/api/supabaseWorkoutRepository.ts`.
- **운동 사전**: `src/data/exerciseDictionary.ts` (875종, 번들 주요 부피). `scratch/extracted_exercises.json`은 생성용.
- **UI**: `src/components/`는 주로 `.jsx`; `App.jsx`가 세 탭을 정적 import (코드 스플리팅 없음).
- **동기화 실패 UI**: `SyncStatusBanner.jsx`. **데모/보내기**: `dummyGenerator.ts`, `exportData.ts`.
...
- **성능**:
  - 운동 사전 lazy load, 탭/보내기 코드 스플리팅, JSZip dynamic import.

## 13. 성능 및 초기 로딩

영문 §13과 동일: 프로덕션 JS 단일 청크 ~1.1MB(gzip ~290KB), 875종 사전 인라인·탭 미분리·게스트 hydrate가 주요 원인. 정적 SPA 배포는 **Vercel** 권장. 1.0 웹 릴리스의 알려진 트레이드오프로 문서화함.
