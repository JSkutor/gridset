...
- **현재 상태 (2026-06-01)**:
  ...
  - 남은 JS/JSX 축은 `store`, `components`, 테스트 파일.

- **다음 작업 순서 제안**:
  1. `src/store/useWorkoutStore.js`와 `src/store/slices/*.js` 타입 설계
     - Reasoning: **high**
  ...
  2. `src/store/workoutPersistenceMigration.js` → `.ts`
     - Reasoning: **high**
