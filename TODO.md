...
- **현재 상태 (2026-06-02)**:
  ...
  - `api/supabaseWorkoutRepository.ts`, `store/useWorkoutStore.ts`, `store/types.ts`, `store/slices/authSlice.ts`, `store/slices/exerciseSlice.ts`, `store/slices/routineSlice.ts`, `store/slices/workoutLogSlice.ts` 전환 완료.
  - store 타입 표면(`WorkoutStore`, slice/action 타입, remote sync 타입)을 추가했고, JS로 남은 store 경계는 persisted migration만 남음.
  - 남은 JS/JSX 축은 `store/workoutPersistenceMigration`, routine hooks, `components`, 테스트 파일.
  - 최근 검증: `npm run typecheck`, `npm run lint`, `npm run test -- src/store`, `npm run build` 통과.
  - 최근 커밋: `fa0e0f0 refactor: introduce TypeScript store migration`

- **다음 작업 순서 제안**:
  1. `src/store/workoutPersistenceMigration.js` → `.ts`
     - Reasoning: **high**
